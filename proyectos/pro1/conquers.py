...
entradas: ()
salida: None
restricciones: none (usa variable global potencias)
funcionamiento: guarda potencias en potencias.txt en formato JSON
"""
...
entradas: ()
salida: None
restricciones: none (lee archivo potencias.txt si existe)
funcionamiento: carga potencias desde potencias.txt al arreglo global potencias
"""
...
entradas: ()
salida: None
restricciones: none (usa variable global vida_territorios)
funcionamiento: guarda vida_territorios en vida_territorios.txt en formato JSON
"""
...
entradas: ()
salida: None
restricciones: none (lee archivo vida_territorios.txt si existe)
funcionamiento: carga vida_territorios desde archivo a la variable global
"""
...
entradas: ()
salida: None
restricciones: none (usa variable global lista_registro)
funcionamiento: guarda lista_registro en registro.txt en formato JSON
"""
...
entradas: ()
salida: None
restricciones: none (lee registro.txt si existe)
funcionamiento: carga registro.txt a lista_registro global
"""
def cargar_registro():
    global lista_registro
...
entradas: ()
salida: None
restricciones: none (modifica lista_registro global)
funcionamiento: limpia el registro y vacía el archivo registro.txt
"""
...
"""
entradas: ()
salida: timestamp (str)
restricciones: none
funcionamiento: devuelve el tiempo actual formateado
"""
def generar_tiempo():
...
"""
entradas: (potencia, territorio, coor)
salida: None
restricciones: potencia str, territorio str, coor lista de coordenadas
funcionamiento: añade un registro de ataque a lista_registro y lo guarda
"""
...
"""
entradas: (potencia, cantidad)
salida: None
restricciones: potencia str, cantidad int
funcionamiento: añade registro de compra y guarda
"""
...
"""
entradas: (potencia, cambio)
salida: None
restricciones: potencia str, cambio bool
funcionamiento: añade registro de cambio de estado y guarda
"""
...
entradas: (potencia)
salida: None
restricciones: potencia str
funcionamiento: añade registro de muerte y guarda
"""
...
salida: None
restricciones: potencia str
funcionamiento: añade registro de territorio (usa potencias) y guarda
"""
...
"""
entradas: (e_x1,e_x2,e_x3,e_y1,e_y2,e_y3) - tk.Entry objetos con valores numéricos
salida: (cx, cy) floats
restricciones: las entradas deben contener números convertibles a float
funcionamiento: convierte DMS (grados,min,seg) a coordenadas decimales
"""
...
"""
entradas: (coord_x, coord_y) - listas/tuplas (grados,min,seg)
salida: (cx, cy) floats transformados para el mapa
restricciones: coord_x/coord_y deben tener 3 elementos numéricos
funcionamiento: convierte DMS a unidades usadas para dibujar en mapa (escala interna)
"""
def convertir_a_xy_mapa(coord_x, coord_y):
    ...
    return cx, cy
"""
entradas: dos coordenadas
salidas: dos coordenadas
restricciones: no puede recibir nada que no sea un número
funcionamiento: convierte las coordenadas del mundo al mapa para ser escritas
"""
...
"""
entradas: (p1, p2) - puntos DMS/estructuras de rectángulo
salida: (xmin, xmax, ymin, ymax)
restricciones: p1 y p2 deben ser tuplas/listas con 3 componentes cada una
funcionamiento: normaliza dos puntos como rectángulo ordenado
"""
...
"""
entradas: (x, y, rect) rect = (xmin,xmax,ymin,ymax) en coordenadas canvas
salida: bool
restricciones: rect debe ser cuadripla de valores comparables con x,y
funcionamiento: determina si un punto está dentro de un rectángulo
"""
...
"""
entradas: (territorio) str
salida: número (extensión estimada)
restricciones: territorio debe existir en MAPA
funcionamiento: calcula extensión aproximada de un territorio por su rectángulo
"""
...
"""
entradas: (nombre) str
salida: número (suma de extensiones de todos sus territorios)
restricciones: nombre debe ser potencia presente en potencias
funcionamiento: suma extensiones de territorios de una potencia
"""
...
entradas: ()
salida: None
restricciones: none (modifica globals)
funcionamiento: inicializa vida_territorios con 100% para cada ciudad y reset potencias
"""
...
entradas: (territorio) str
salida: None
restricciones: territorio debe existir en vida_territorios
funcionamiento: reduce vida del territorio en 10 (por impacto o compra)
"""
...
entradas: ()
salida: None
restricciones: usa potencias y vida_territorios globals
funcionamiento: marca potencias sin territorios como derrotadas y crea registro
"""
...
entradas: ()
salida: bool
restricciones: none (usa MAPA)
funcionamiento: verifica si hay traslapes entre rectángulos de territorios; True si OK
"""
...
"""
entradas: () usa entrada_potencia_r y entrada_territorio (tk.Entry)
salida: None
restricciones: entradas deben contener nombres válidos; potencias globales actualizadas
funcionamiento: asigna un territorio al arreglo de potencias si es válido
"""
...
"""
entradas: () usa entrada_potencia (tk.Entry)
salida: None
restricciones: entrada debe ser nombre no duplicado
funcionamiento: agrega nueva potencia a la lista global potencias
"""
...
"""
entradas: () usa entrada_potencia (tk.Entry)
salida: None (actualiza status_var)
restricciones: entrada debe ser nombre de potencia existente
funcionamiento: construye y muestra el estado de la potencia solicitada
"""
...
entradas: () usa entrada_potencia (tk.Entry)
salida: None
restricciones: entrada potencia debe existir
funcionamiento: cambia estado activo/inactivo de una potencia y guarda
"""
def cambiar_estado():
    global potencias
...
"""
entradas: (nombre_potencia) str
salida: vida promedio (float) o 0 si no tiene territorios
restricciones: nombre_potencia debe ser string de potencia existente
funcionamiento: calcula vida promedio de territorios de una potencia
"""
...
entradas: (ciudad) str
salida: None
restricciones: ciudad debe ser nombre de ciudad en potencias (si aplica)
funcionamiento: actualiza vida y contadores de la potencia propietaria cuando una ciudad recibe daño
"""
...
"""
entradas: () usa entrada_potencia y entrada_cantidad_misiles (tk.Entry)
salida: None
restricciones: cantidad debe ser entero >0 y <=1000 y múltiplo de 100; potencia debe existir y poseer territorios
funcionamiento: compra misiles, resta vida del primer territorio de la potencia y registra acción
"""
def comprar_misiles():
...
"""
entradas: ()
salida: lista de nombres de potencias ordenadas por extensión (desc)
restricciones: potencias debe estar poblada
funcionamiento: calcula ranking por extensión total de territorios
"""
def rank_extension():
    global potencias
...
"""
entradas: ()
salida: lista de nombres de potencias ordenadas por vida (desc)
restricciones: potencias debe estar poblada
funcionamiento: calcula ranking por porcentaje de vida de potencias
"""
def rank_porcentaje():
    global potencias
...
salida: None
restricciones: none (modifica globals)
funcionamiento: reinicia potencias, vida de territorios y registro
"""
def reiniciar_juego():
    global potencias
...
entradas: (nombre) str
salida: bool
restricciones: nombre debe ser string
funcionamiento: verifica si la potencia existe, está viva y activa
"""
...
"""
entradas: () usa entradas de coordenadas y entrada_potencia (tk.Entry)
salida: None (actualiza resultado_var, globals y archivos)
restricciones: entradas deben ser válidas y potencia activa con misiles; coordenadas dentro de rango
funcionamiento: procesa un disparo, reduce misiles, vida de territorios y registra el evento
"""
def disparar():
